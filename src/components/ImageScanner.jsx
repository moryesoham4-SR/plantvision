      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
        ...
        <div className="flex bg-slate-100 p-1 rounded-lg text-xs font-semibold">
          <button
            ...
            className={`px-3 py-1 rounded-md transition ${activeMode === "samples" ? "bg-white text-emerald-700 shadow-xs" : "text-slate-600 hover:text-slate-900"}`}
          >
            ⚡ 1-Click Samples
          </button>
          <button
            onClick={() => { setActiveMode("upload"); setCameraActive(false); }}
            className={`px-3 py-1 rounded-md transition ${activeMode === "upload" ? "bg-white text-emerald-700 shadow-xs" : "text-slate-600 hover:text-slate-900"}`}
          >
            📁 File Upload
          </button>
          <button
            onClick={() => { setActiveMode("camera"); startCamera(); }}
            className={`px-3 py-1 rounded-md transition ${activeMode === "camera" ? "bg-white text-emerald-700 shadow-xs" : "text-slate-600 hover:text-slate-900"}`}
          >
            📷 Live Camera
          </button>
        </div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {currentPlantSamples.map((sample, idx) => (
              <button
                ...
                className={`p-3 rounded-xl border text-center transition flex flex-col items-center justify-center gap-1.5 hover:scale-[1.02] ${sample.color}`}
              >
