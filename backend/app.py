...
with st.sidebar:
    ...
    st.caption(fAccount: @{user['username']} | Email: {user['email']})
    st.markdown(---)
...
if selected_page == 🔬 Disease Scanner:
 ...
 sample_path = config.SAMPLES_DIR / selected_sample_file
 ...
 st.image(display_img, caption=fInput Image (Preprocessed to {meta['processed_size'][0]}x{meta['processed_size'][1]} RGB), use_column_width=True)
 ...
 with st.spinner(fRunning inference with {config.PLANTS[selected_plant_id]['name']} Model...):
 ...
 save_filename = fscan_{user['id']}_{int(meta['processed_size'][0])}_{selected_plant_id}_{sample_name.replace(' ', '_')}.jpg
 ...
 data=pdf_bytes,
 file_name=fPlantVision_Report_{res['plant_id']}_{res['predicted_disease'].replace(' ', '_')}.pdf,
...
# ----------------- PAGE 2: HISTORY -----------------
 ...
 st.write(fFound **{len(scans)}** scan records.)
 for scan in scans:
 ...
 try:
 st.image(scan[image_path], caption=f{scan['plant']} Leaf, use_column_width=True)
 except Exception:
 ...
 del_btn = st.button(🗑️ Delete Record, key=fdel_{scan['id']})
 if del_btn:
