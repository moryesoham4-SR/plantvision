...
def predict_potato(tensor_input: np.ndarray, image_hint: str = "") -> Dict[str, Any]:
    if config.USE_MOCK:
        return get_mock_prediction("potato", image_hint)

    from models.model_loader import get_or_load_model
    model = get_or_load_model("potato")
    ...
    classes = config.PLANTS["potato"]["classes"]
    predicted_disease = classes[class_idx]
    disease_info = DISEASE_KNOWLEDGE_BASE["potato"].get(predicted_disease, {})

    probabilities = {classes[i]: float(predictions[0][i]) for i in range(len(classes))}

    return {
        "plant_id": "potato",
        "plant_name": config.PLANTS["potato"]["name"],
        "scientific_name": config.PLANTS["potato"]["scientific_name"],
