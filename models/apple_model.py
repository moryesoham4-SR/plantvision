...
def predict_apple(tensor_input: np.ndarray, image_hint: str = "") -> Dict[str, Any]:
    if config.USE_MOCK:
        return get_mock_prediction("apple", image_hint)

    from models.model_loader import get_or_load_model
    model = get_or_load_model("apple")
    ...
    classes = config.PLANTS["apple"]["classes"]
    predicted_disease = classes[class_idx]
    disease_info = DISEASE_KNOWLEDGE_BASE["apple"].get(predicted_disease, {})

    probabilities = {classes[i]: float(predictions[0][i]) for i in range(len(classes))}

    return {
        "plant_id": "apple",
        "plant_name": config.PLANTS["apple"]["name"],
        "scientific_name": config.PLANTS["apple"]["scientific_name"],
